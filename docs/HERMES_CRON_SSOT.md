...
daily-dose 08:00 · 
### Trial closeout watch (anti quiet-death)
| Job | Schedule (PT) | Deliver | Notes |
|-----|---------------|---------|-------|
...
| `phase6-marketdata-btc-1d` `c2753e10a9c9` | **CW-1 D3/D4 sensor** · thin universe 1d → `data/marketdata.db` (held∪basket∪tryout∪BTC/ETH/PAXG) + freshness + refresh `regime_cash_status` · **07:40 PT daily** · **`deliver=local`** (full dump in `logs/marketdata/` only; empty stdout — no TG) · **no knobs / no unpark** · live cutover: arm-switch + run-phase prefer store · plan `docs/plans/2026-09-22-marketdata-ohlcv-schema-design.md` |
...
| `phase6-basket-swap-cf-shadow` `b970bb3247e9` | Measure-only CF/arms board · **11:30/23:30 PT** · **`deliver=local`** (Brad 2026-09-23: no TG while `live_membership_swaps` OFF — paper dual_agree is not a live swap decision). CF-only `modify_selector` already silent in body. Re-enable TG only with dated GO when a real membership decision is in play. preferred from decision file (regime switch may flip) →2026-09-28 |
...
- Schedule: `10 8 * * *` America/Los_Angeles (**after** `daily-dose-telegram` 08:00 freeze)
- Script: `~/.hermes/scripts/run_daily_dose_jev_ab.sh` → `phase6/scripts/run_daily_dose_jev_ab_cron.sh`
