#!/usr/bin/env python3
"""
R-004: Layer 2 - Inverse Vol + Liquidity + Holding Proportional Bias

Tests the value of adding holding proportional bias (sticky scaling)
on top of inverse volatility + liquidity bias.
"""
...
def run_layer2_backtest():
    print("=" * 75)
    print("R-004: LAYER 2 - INVERSE VOL + LIQUIDITY + HOLDING BIAS")
    print("=" * 75)

    print("\n[1] Loading historical OHLCV data...")
    hist_data = load_historical_data()

    print("\n[2] Running simulation with Holding Proportional Bias (last 120 days)...")
    ...
    for i, row in enumerate(test_data):
        ...
        w1 = apply_liquidity_bias(base_weights, liq_scores, liquidity_weight=0.3)

        # Add Holding Proportional Bias (sticky to existing holdings)
        final_weights = apply_holding_proportional_bias(w1, current_holdings, holding_bias=0.35)
