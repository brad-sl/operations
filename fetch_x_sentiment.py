...
CACHE_FILE = BASE_DIR / "phase6" / "data" / "sentiment" / "x_sentiment_cache.json"
...
def fetch_batched(pairs: List[str], kw_dict: Dict[str, str], bearer: str, batch_size: int = 5, max_res: int = 100) -> Dict[str, List[Dict]]:
...
def main():
    ...
    print(f"🔍 Fetching X for dynamic basket of {len(pairs)} pairs...")

    if len(pairs) > 5:
        print("   Batched mode enabled (better post counts per pair for significance)")
        pair_posts = fetch_batched(pairs, kw_dict, bearer, batch_size=5)
    else:
