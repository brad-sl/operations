...
class SpendTracker:
    ...
    def __init__(self, config: TradingConfig, total_capital: float = 10000.0):
        self.config = config
        self.total_capital = total_capital
        self.daily_spend_usd = 0.0
        self.daily_orders = []  # List of order results per day
        self.session_start = datetime.now(timezone.utc)
        self.per_trade_limit_pct = 0.01  # 1% per trade max
    ...
    def within_daily_loss_limit(self) -> bool:
        """Check if daily loss exceeds circuit breaker (5% of capital)"""
        daily_pnl = sum(
    ...
    def within_per_trade_limit(self, usd_amount: float) -> bool:
        """Enforce 1% per-trade risk limit"""
        max_per_trade = self.total_capital * self.per_trade_limit_pct
        return usd_amount <= max_per_trade

    def round_to_precision(self, quantity: float, product_id: str) -> float:
        """Round quantity to Coinbase quote precision / min size (basic impl)"""
        # Common Coinbase precisions (extend via config or API in prod)
        precisions = {
            "BTC-USD": 0.00000001,
            "ETH-USD": 0.000001,
            "SOL-USD": 0.0001,
        ...
        step = precisions.get(product_id, 0.0001)
...
class OrderExecutor:
    ...
    def __init__(
        ...
        # Note: CoinbaseAdvancedClient doesn't expose sandbox attribute,
        # so we trust the caller to pass the correct wrapper configured properly
        if sandbox_mode:
            self.logger.info("✅ Sandbox mode enabled for OrderExecutor")
    ...
    def execute_signal(self, signal: Dict) -> ExecutionResult:
        ...
        try:
            ...
            # Calculate quantity and apply safeguards
            raw_quantity = self.order_size_usd / current_price
            quantity = self.spend_tracker.round_to_precision(raw_quantity, self.product_id)

            # Per-trade 1% limit
            if not self.spend_tracker.within_per_trade_limit(self.order_size_usd):
                max_trade = self.spend_tracker.total_capital * self.spend_tracker.per_trade_limit_pct
                raise ValueError(
                    f"Per-trade limit exceeded: ${self.order_size_usd:.2f} > 1% (${max_trade:.2f}) of capital"
                )

            # Check spend limits before creating order
            if not self.spend_tracker.within_daily_budget(self.order_size_usd):
                daily_spent = self.spend_tracker.daily_spend_usd or 0
                raise ValueError(
            ...
            if not self.spend_tracker.within_position_limit(self.product_id, quantity):
                position_limit = self.config.position_limits.get(self.product_id, 0.05) or 0.05
                raise ValueError(
                    f"Position size {quantity:.8f} {self.product_id.split('-')[0]} "
                    f"exceeds limit {position_limit}"
                )

            if not self.spend_tracker.within_daily_loss_limit():
                max_loss = self.spend_tracker.total_capital * self.spend_tracker.max_daily_loss_pct
                raise ValueError(
                    f"Daily loss limit (5% = ${max_loss:.2f}) reached. Trading halted for today."
                )

            # SL/TP handling note: SL/TP enforced via liquidation_manager post-fill or OCO orders
            # For now, record intended SL/TP in result metadata (future: attach to order)
