#!/usr/bin/env python3
"""
6-month portfolio counterfactual: run-lifecycle stack vs baselines.
...
Run:
  cd /home/brad/projects/crypto-trading-bot && PYTHONPATH=. python3 \\
    scripts/phase6/backtest_run_lifecycle_6mo_cf.py

Report: data/state/run_lifecycle_6mo_cf_report.json
"""
...
OUT = ROOT / "data/state/run_lifecycle_6mo_cf_report.json"
UA = {"User-Agent": "Mozilla/5.0 (compatible; phase6-6mo-cf/1.0)"}
...
def main() -> int:
    ...
    report = {
        ...
        "params": {
            ...
            "note": (
                ...
                "Not a promise of live returns — structure CF over ~6 months."
            ),
        },
        "leaderboard": sorted(
            [{"arm": k, **v["metrics"]} for k, v in arms.items()],
            key=lambda x: x.get("total_return_pct") or -999,
            reverse=True,
        ),
        "arms": arms,
    ...
    print("\n======== LEADERBOARD (6mo CF) ========")
    print(f"{'arm':24} {'ret%':>8} {'maxDD%':>8} {'sharpe':>7} {'cash%':>7} {'buys':>5} {'end$':>10}")
    for row in report["leaderboard"]:
        print(
            f"{row['arm']:24} {row.get('total_return_pct', 0):8.2f} "
            f"{row.get('max_drawdown_pct', 0):8.2f} {row.get('sharpe_daily_ann', 0):7.2f} "
            f"{row.get('avg_cash_pct') or 0:7.1f} "
            f"{row.get('n_buys', 0):5} {row.get('end_usd', 0):10.2f}"
        )
    print(f"\nReport → {OUT}")
    print(
