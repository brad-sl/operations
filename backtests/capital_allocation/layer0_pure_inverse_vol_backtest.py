#!/usr/bin/env python3
"""
R-002: Layer 0 - Pure Inverse Volatility Baseline

No liquidity bias, no sentiment, no holding proportional bias.
This is the clean baseline for comparison against sentiment-enhanced runs.
"""
...
def run_layer0_backtest():
    print("=" * 75)
    print("R-002: LAYER 0 - PURE INVERSE VOLATILITY BASELINE")
    print("=" * 75)

    print("\n[1] Loading historical OHLCV data...")
    hist_data = load_historical_data()

    print("\n[2] Running pure inverse-vol simulation (last 120 days)...")
    ...
    for i, row in enumerate(test_data):
        ...
        # Pure inverse volatility (no other biases)
        base_weights = {}
        total_inv = 0
        for p in PAIRS:
            if p in hist_data:
        ...
        if total_inv > 0:
            final_weights = {p: v / total_inv for p, v in base_weights.items()}
        else:
            final_weights = current_holdings
