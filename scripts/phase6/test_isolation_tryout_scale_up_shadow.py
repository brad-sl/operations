...
def _pos(pair="LINK-USD", value=25.0, entry=10.0, mark=10.25, unreal=0.025):
    return {
        "pair": pair,
        "value_usd": value,
        "entry_price": entry,
...
def test_would_scale_when_gates_clear():
    ...
    assert d.step_usd == 25.0
    assert "earned_mid_flight_step" in d.reasons


def test_would_scale_legacy_75_shell_still_in_band():
    """Transition: residual ~$75 tryouts still countable; step is $25 not $75."""
    cfg = load_cfg()
    now = datetime(2026, 9, 5, 20, 0, tzinfo=timezone.utc)
    entry_ts = (now - timedelta(hours=5)).isoformat()
    pos = _pos(value=75.0)
    ...
    assert d.status == "would_scale", d
    assert d.step_usd == 25.0
    assert abs(d.detail.get("total_after_usd", 0) - 100.0) < 0.01
...
def test_defaults_match_25_shell():
    cfg = load_cfg()
    assert cfg["tryout_min_usd"] <= 25.0
    assert cfg["tryout_max_usd"] >= 25.0
    assert cfg["step_usd"] == 25.0
    assert cfg["max_total_after_step_usd"] == 100.0
    assert cfg["live_apply"] is False


def test_scale_decision_dict():
    d = ScaleDecision("X", "would_scale", 25, 0.02, 3.0, 1, True, 25.0, ["ok"])
