#!/usr/bin/env python3
"""CLI: marketdata D1/D2 — init, BTC 1d ingest, status, regime smoke.

  PYTHONPATH=. python3 scripts/phase6/run_marketdata_btc_1d.py ingest
  PYTHONPATH=. python3 scripts/phase6/run_marketdata_btc_1d.py status
  PYTHONPATH=. python3 scripts/phase6/run_marketdata_btc_1d.py regime
"""
...
def main() -> int:
    ap = argparse.ArgumentParser(description="Marketdata BTC 1d (D1/D2)")
    ap.add_argument(
        "cmd",
        choices=("init", "ingest", "status", "regime", "all"),
        help="init schema | ingest BTC 1d | status | detect_regime | all",
    )
    ap.add_argument("--db", default="", help="override marketdata.db path")
    ap.add_argument("--lookback-days", type=int, default=400)
    ap.add_argument("--no-api", action="store_true", help="local JSON only")
    ...
    from phase6.core.marketdata_ingest import ingest_btc_1d, write_local_btc_mirror
    ...
    if args.json or args.cmd in ("status", "regime") and args.json:
        print(json.dumps(out, indent=2, default=str))
    else:
        print(json.dumps(out, indent=2, default=str))

        # short human board for cron
        if "regime" in out:
            r = out["regime"]
            print(
    ...
    return 0 if out.get("ingest", {}).get("ok", True) is not False or args.cmd != "ingest" else 1
