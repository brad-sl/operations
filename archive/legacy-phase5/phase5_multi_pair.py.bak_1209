...
class Phase5Harness:
    ...
    def _fetch_all_pairs_batch(self):
        ...
        for chunk_idx, chunk in enumerate(chunks, 1):
            try:
                products = self.cb_client.client.get_products(product_ids=chunk)
                if hasattr(products, 'products'):
                    for product in products.products:
                        all_prices[product.id] = float(product.price)
                else:
                    for product in products:
                        all_prices[product['id']] = float(product['price'])
                self.logger.info(f"✅ Batch {chunk_idx}/{len(chunks)}: {len(chunk)} pairs")
            except Exception as e:
                ...
                for pair in chunk:
                    try:
                        ...
                        if hasattr(single, 'products'):
                            all_prices[single.products[0].id] = float(single.products[0].price)
