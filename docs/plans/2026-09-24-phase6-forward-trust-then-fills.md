...
**Status:** LOCKED 2026-09-24 ~17:20 PT (Brad). Wave 0/1 executing as measure-only scorecard. No knob writes.

## Decisions locked

- **Q1:** Prove the current shell on the next three **21:00 PT** slots (2026-09-24, 09-25, 09-26). No loosen tonight. No force-rebalance.
- **Q2:** Do not pre-commit a floor cut or RSI widen. After each slot, the scorecard hint applies: quiet sensor → **W2-D** (stay idle); sent-clear + RSI-only → **W2-C or W2-A**, still a dated GO before any config write; bug exit → stop. Floor cut (W2-B) is not the default.
- **Q3:** Do not promote a shadow now. **If a shadow later beats the live shell on the bar below, the direction is shadow — not the current shell.** Auto-promote stays off: the cron that sees the bar pages a one-screen card; wiring waits for that same-day confirm so a bug-contaminated sample cannot flip knobs alone.

### Shadow “superior” bar (head-to-head vs live shell, after fees)

| Arm | Superior means | Now |
|-----|----------------|-----|
| Knife | n_allow ≥ 20 and one arm has strictly lower SL rate **and** mean net R ≥ rsi_only | n=2, not superior |
| Jev select-few | n_closed ≥ 20 and mean forward R **beats** RSI+sent on the same names | n_closed=0, not superior |
| Scale-up | n ≥ 8 and excess (scale − tryout) **> 0** after fees | excess **negative**, not superior |
| RSI-event X | would-buy later matches a clean non-bug fill, not a spray | 0 triggers, not superior |

None of these are superior today. Q3 does not change Wave 1.
